...
MPMT4_args::MPMT4_args():Thread_args(){
  data_sock=0;
  utils=0;
  job_queue=0;
  m_data=0;
...
bool MPMT4::Initialise(std::string configfile, DataModel &data){
...
void MPMT4::CreateThread(){

  MPMT4_args* tmparg=new MPMT4_args();
 ...
 void MPMT4::DeleteThread(unsigned int pos){
...
void MPMT4::Thread(Thread_args* arg){

  MPMT4_args* args=reinterpret_cast<MPMT4_args*>(arg);
  ...
  if(args->items[0].revents & ZMQ_POLLIN){
    ...
    if(args->no_data){
      ...
      MPMT4Messages* tmp_msgs= new MPMT4Messages;
...
bool MPMT4::ProcessData(void* data){

  //printf("in process data\n");
  MPMT4Messages* msgs=reinterpret_cast<MPMT4Messages*>(data);
  //printf("d1\n");  
