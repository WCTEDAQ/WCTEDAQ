...
bool Sorting::SortData(void* data){
  ...
  */
  ...
  for(unsigned int i=0; i< args->unsorted_data->extra_hits.size(); i++){
    bin=((args->unsorted_data->extra_hits.at(i).GetCoarseCounter() & 4194303U)<<3) | (args->unsorted_data->extra_hits.at(i).GetFineTime() >>13);
  ...
  for(unsigned int i=0; i< args->unsorted_data->extra_waveforms.size(); i++){
    ...
    tmp->extra_waveforms.at(arr_count[bin]-1)=args->unsorted_data->extra_waveforms.at(i);
    arr_count[bin]--;
  }
...
void Sorting::LoadConfig(){
  unsigned int pre_sort_wait;
  if(!m_variables.Get("verbose",m_verbose)) m_verbose=1;
  if(!m_variables.Get("pre_sort_wait",pre_sort_wait)) pre_sort_wait=2; 
